...
| Last completed | CR-2 recorded (no manual confirmation; Tk 500 bKash advance). Building M5 admin now against the current model ("M5 now, adapt after"). Before: M4.3–M4.5 booking flow, 112 passed (2026-09-11) |
| Next task | M5.1 admin login/session → M5.2 dashboard → M5.3 bookings list → M5.4 calendar (block/unblock) → M5.5 tests. Then M4.6 (CR-2 model change, needs C-28) and CR-1 M2.6+. |
