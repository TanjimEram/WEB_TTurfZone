...
| Last completed | M4.3–M4.5: customer booking flow — details form, `POST /book` (CSRF + re-validation), `booking_success.html` (session-read, D-11), 10 route tests. 112 passed, 4 skipped. Browser-verified end to end at 375px (2026-09-11). Before that: M2.3, M2.2, M4.2 picker |
| Next task | **Decision point.** Options with no CR-1 sign-off: **M5** (admin: login, dashboard, bookings list confirm/reject/cancel, calendar block/unblock) or **M7.1–M7.3/M7.5** (assets, SEO, 404/500, a11y — needs owner assets). Or start CR-1 with **M2.6/M2.7** (schema + holds) if repricing (C-27) is agreed. Recommendation: M5. |
