...
| Last completed | **M5 admin complete** (login, dashboard, bookings list + confirm/reject, calendar + block/unblock). 143 passed, 4 skipped. Browser-verified (2026-09-11). Before: CR-2 recorded, M4.3–M4.5 booking flow |
| Next task | **Decision.** (a) **M4.6** — apply CR-2: bookings auto-confirm, no PENDING, success page wording — needs C-28 answered (interim free vs form closed). (b) **M2.6** — CR-1 schema (needs C-27 repricing). (c) **M7.5 / M7.3** — 404·500, a11y, SEO (no owner assets needed). (d) **M6** — deploy the current build (blocked on hosting purchase). |
