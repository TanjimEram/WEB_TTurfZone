...
| Last completed | M2.3: `services/bookings.py` — `normalize_phone`, `create_booking_request`, `confirm_booking`, `reject_booking`, `block_slot`, `unblock_slot` + 26 tests. 102 passed, 4 skipped (2026-09-11). Before that: M2.2, M4.2 picker, M4.1, CR-1 docs, M3 |
| Next task | **Decision point.** Execution order says M2.6/M2.7 (CR-1 schema: PENDING_PAYMENT + holds + payments/notifications tables + migration) next — but that is the first CR-1 build task and CR-1 repricing (C-27) is not agreed. Alternatives that need no CR-1 sign-off: M4.3–M4.5 (manual-mode booking form + POST /book, current schema) or M5 (admin). Confirm which to run. |
